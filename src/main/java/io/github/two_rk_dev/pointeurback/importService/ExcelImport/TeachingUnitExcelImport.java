...
@Component
public class TeachingUnitExcelImport implements FileImport {
    private final TeachingUnitServiceImpl teachingUnitServiceImpl;
    private final ObjectMapper mapper;

    public TeachingUnitExcelImport(TeachingUnitServiceImpl teachingUnitServiceImpl, ObjectMapper mapper) {
    ...
    public boolean supports(String entityType, String fileExtension) {
        return "teachingunit".equalsIgnoreCase(entityType) && ("xlsx".equalsIgnoreCase(fileExtension) || "xls".equalsIgnoreCase(fileExtension));
    }
    ...
    public void importData(MultipartFile file) {
        try (InputStream in = file.getInputStream(); XSSFWorkbook workbook = new XSSFWorkbook(in)) {
            ...
            CreateTeachingUnitDTO[] dtos = mapper.convertValue(mappedRows, CreateTeachingUnitDTO[].class);
            teachingUnitServiceImpl.saveTeachingUnits(dtos);
        } catch (Exception e) {
            throw new RuntimeException("Failed to import teaching unit Excel: " + e.getMessage(), e);
        }
    }
}
