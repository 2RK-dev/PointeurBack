...
@Component
public class TeacherExcelImport implements FileImport {
    private final TeacherServiceImpl teacherServiceImpl;
    private final ObjectMapper mapper;

    public TeacherExcelImport(TeacherServiceImpl teacherServiceImpl, ObjectMapper mapper) {
    ...
    public boolean supports(String entityType, String fileExtension) {
        return "teacher".equalsIgnoreCase(entityType) && ("xlsx".equalsIgnoreCase(fileExtension) || "xls".equalsIgnoreCase(fileExtension));
    }
    ...
    public void importData(MultipartFile file) {
        try (InputStream in = file.getInputStream(); XSSFWorkbook workbook = new XSSFWorkbook(in)) {
            ...
            CreateTeacherDTO[] dtos = mapper.convertValue(mappedRows, CreateTeacherDTO[].class);
            teacherServiceImpl.saveTeachers(dtos);
        } catch (Exception e) {
            throw new RuntimeException("Failed to import teacher Excel: " + e.getMessage(), e);
        }
    }
}
